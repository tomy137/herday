...
export default function Journal() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.journal.upsert(dateStr, draft);
  ...
  const todayMeta = new Intl.DateTimeFormat(undefined, { weekday: 'short', day: 'numeric', month: 'short' })
    .format(today())
    .replace(/\./g, '')
    .toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-warm-50">
      <div className="flex items-center justify-between px-[22px] pt-3.5">
        ...
        <span className="hd-meta text-warm-500">{t('full.entry')} · {todayMeta}</span>
        <button
          type="button"
          onClick={save}
      ...
      <div className="flex flex-col gap-[22px] px-[22px] pb-12 pt-5">
        <div>
          ...
          <h1 className="text-[28px] font-bold leading-tight tracking-tight text-ink">{t('full.title')}</h1>
