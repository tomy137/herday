  ...
  if (step === 0) {
    return (
      <OnbStep index={0} total={TOTAL} eyebrow={t('step1.eyebrow')} title={t('step1.title')} cta={t('step1.cta')} onPrimary={next}>
        <p className="mb-7 text-[14.5px] leading-relaxed text-warm-500">{t('step1.body')}</p>
        <div>
          <label className="hd-caps mb-1.5 block text-warm-500">{t('step1.email_label')}</label>
          <div className="border-b-[0.5px] border-warm-300 py-2.5 text-[15px] text-ink">{user?.email}</div>
          <p className="hd-meta mt-2.5 text-warm-400">{t('step1.email_hint')}</p>
        </div>
      </OnbStep>
    );
  }
    ...
    return (
      <OnbStep index={2} total={TOTAL} eyebrow={t('step3.eyebrow')} title={t('step3.title')} cta={t('step3.cta')} onPrimary={next} onBack={back}>
        ...
        <div className="rounded-[14px] bg-warm-100 px-2 pb-2 pt-4">
          <CycleGraph dayInCycle={13} cycleLength={28} phase="pre_ovulatory" />
        </div>
        <div className="mt-[18px] flex flex-col gap-1.5">
          {PHASE_ORDER.map((id) => (
            <div key={id} className="flex items-center gap-2.5 py-1.5 text-[13px]">
              <span className="h-2.5 w-2.5 rounded-[2px]" style={{ background: PHASE_HEX[id] }} />
              <span className="flex-1 text-ink">{tPhases(`${id}.short`)}</span>
